...
class Inc_Learning_Appr:
    ...
    def __init__(self, model, device, nepochs=2, ftepochs=1, lr=0.05, lr_min=1e-4, lr_factor=3, lr_patience=5, clipgrad=10000,
                 ...
                 eval_on_train=False, logger: ExperimentLogger = None,num_ini_classes=10,num_experts=5, num_tasks=20,random_seed=0,datasets=['cifar100_icarl'],collapse_alpha=0,inc_sub_rate=0.5,exemplars_dataset: ExemplarsDataset = None):
        self.model = model
        self.device = device
        ...
        self.num_ini_classes = num_ini_classes
        ...
        self.inc_sub_rate=inc_sub_rate
    ...
    def train(self, t, trn_loader, val_loader, ini_trn_load, inc_trn_loader):#运行主程序后，随着appr.train(t, trn_loader[t], val_loader[t])会到这里
        """Main train structure"""
        self.pre_train_process(t, trn_loader[t])  # 这里会进去本文件的pre_train_process
        self.train_loop(t, trn_loader, val_loader, ini_trn_load, inc_trn_loader)#不会进本文件的train_loop了，而是会进实验对应选择的算法内的train_loop，比如会进seed内的train_loop()
    ...
    def calculate_metrics(self, outputs, targets):
        ...
        pred = torch.zeros_like(targets.to(self.device))
        # Task-Aware Multi-Head（任务可知条件下：即知道当前任务中有几个类，且知道这几个类是什么。产生预测结果时，只会在当前任务中包含的类里选预测概率更大的类作为当前的预测类别）
        for m in range(len(pred)):
            this_task = (self.model.task_cls.cumsum(0) <= targets[m]).sum()
            pred[m] = outputs[this_task][m].argmax() + self.model.task_offset[this_task]
        hits_taw = (pred == targets.to(self.device)).float()
        # Task-Agnostic Multi-Head（任务不可知条件下：即不知道当前任务中有几个类，这些类是什么，所以在产生预测结果时是从softmax中的所有类的预测概率里面选概率最大的类作为预测类别）
